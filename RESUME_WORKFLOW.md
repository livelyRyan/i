...
1. **`resume-user.md` 是唯一真源（Single Source of Truth）**：所有信息只在这里维护一次；不再区分投递版。
...
- `resume-user.md`：投递简历真源（建议 1 页为默认目标）
- `resume.en.md`：英文版（如果需要双语）
- `content/`：网站扩展内容（不与 `resume-user.md` 重复）
...
## 2. `resume-user.md` 必须满足的格式约束（给 LLM 的硬规则）
...
### 2.6 单一投递简历
只维护一份 `resume-user.md`，不区分 Agent / Platform 版本；需要突出岗位时在同一份简历中合并表达即可。

## 3. LLM 修改协议（每次改简历都要遵守）

为了让修改可控，`resume-user.md` 使用“可编辑区块标记”。LLM 必须：
...
- 简历页：展示 `resume-user.md`（或其渲染结果），保持与投递版一致。
- 项目页：每个项目单独展开（背景、目标、约束、方案、取舍、结果、复盘）。
- 文章页：展示思考与方法论（可提升可信度与可聊话题）。

### 4.1 去重规则（强制）
- 网站的 Experience 区域不再维护第二份 bullets；**直接渲染 `resume-user.md`** 或引用其片段。
...
由于需要“网站直接渲染 `resume-user.md` + 扩展内容”，推荐：

- 静态站点：**Astro**
  - 优点：原生支持 Markdown 内容与布局组合；对 LLM 维护友好；生成纯静态产物便于部署。
- 导出工具：**Pandoc**
  - `resume-user.md` → `resume.pdf` / `resume.docx`
  - 版式用单栏、简洁 CSS（避免多栏/表格）

> 说明：本规范只定义设计与约束。落地时会补齐 `resume-user.md` 模板、Astro 工程与一键脚本。
